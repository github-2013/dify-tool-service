...
def upload_markdown():
    ...
    # 将Markdown内容写入文件
    with open(f'static/markdown/{md_file_name}', "w", encoding='utf-8') as f:
        f.write(content)

    # 使用subprocess调用markmap-cli将Markdown转换为HTML，并移动到static/html目录
    try:
        result = subprocess.run(['npx', 'markmap-cli', f'static/markdown/{md_file_name}', '--no-open'], capture_output=True, text=True)

        if result.returncode != 0:
            raise subprocess.CalledProcessError(result.returncode, result.args, output=result.stdout, stderr=result.stderr)

        # 尝试将生成的HTML文件移动到static/html文件夹
        os.replace(f'static/markdown/{html_file_name}', f'static/html/{html_file_name}')

        # 返回转换后的HTML文件链接
        # return f'Markdown文件已保存. 点击预览: {url_for("get_html", filename=html_file_name, _external=True)}'
        return f'Markdown 文件已保存，点击预览: http://127.0.0.1:15002/static/html/{html_file_name}'
